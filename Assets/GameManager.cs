...
public class GameManager : MonoBehaviour
{
    ...
    private int fireballFrameThreshold;
    private int fireballCounter;

    public ExpanderControl expander;

    private int expanderFrameThreshold;
    private int expanderCounter;

    private bool playedGameEndSound = false;
    ...
    void Update()
    {
        if ((player1.Score != maxScore) && (player2.Score != maxScore))
        {
            if (fireball.gameObject.activeSelf == false)
            {
                ...
                if (fireballCounter >= fireballFrameThreshold)
                {
                    fireballCounter = 0;
                    fireballFrameThreshold = Random.Range(300, 1200);
            ...
            if (expander.gameObject.activeSelf == false) {
                expanderCounter++;

                if (expanderCounter >= expanderFrameThreshold) {
                    expanderCounter = 0;
                    expanderFrameThreshold = Random.Range(1500, 4800);
                    expander.StartExpander();
                }
            }
        }
    }
    ...
    void OnGUI()
    {
        ...
        if (player1.Score == maxScore)
        {
            ...
            expander.gameObject.SetActive(false);
